...
const { getProject, runQuery } = require('../shared.js');
const { ObjectId } = require('mongoose').Types;

router.use('/:projectId/area', (req, res, next) => {res.projectId = req.params.projectId; next()}, areaAPI)
...
router.get('/', (req, res, next) => { req.params.queryObj = {}; next();}, runQuery,
(req, res, next) => {
    res.status(200).json({ success: true, payload: res.mongoResponse })
});
