...
router.get('/:linkId', determineType, setObj, async(req, res) => {
    let resp;
    resp = res.linkParent.links.find(obj => { return obj._id == req.params.linkId })
...
router.get('/', determineType, setObj, (req, res, next) => {
    res.status(200).json({ success: true, payload: (res.roomId ? res.room.links : res.linkParent.links) })
});
...
router.post('/:linkId', determineType, setObj, async(req, res) => {
    try {
        ...
        {
            if (!res.linkParent.links.includes(req.params.linkId))
                res.linkParent.links.push(req.params.linkId)
            else throw 'ID already in links'
